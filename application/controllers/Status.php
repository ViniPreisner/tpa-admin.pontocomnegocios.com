...
class Status extends MY_Controller {
	...
	public function index($page = false)
	{
        ...
        // DADOS        
		$returnTotalItems = $this->sm->selectAllItemsCount();
        ...
        if ($total_records > 0) {
            $this->data['items'] = $this->sm->getAll($limit_per_page,$start_index,$total_records);
            $this->data['pagination'] = $this->paginationLinks($total_records,$limit_per_page,'status');    
        } else {
		...
		$this->data['content'] = $this->load->view('content/status-lista', $this->data, TRUE);

		$this->data['footer'] = $this->load->view('footer', $this->data, TRUE);
		$this->data['extra_scripts'] = array($this->load->view('js/status', NULL, TRUE));
	...
	public function incluir()
	{
		...
		$this->data['content'] = $this->load->view('content/status-form', $this->data, TRUE);
	...
	public function post()
	{
        ...
        if ($this->input->post('id')) {
            $return = $this->sm->update($data,$this->input->post('id'));
            $this->data['message'] = 'Cadastro alterado com sucesso.';    
        }
        // insert
        else {
            $return = $this->sm->insert($data);
	...
	public function delete()
	{
        ...
        if ($this->sm->delete($this->input->post('id'))) {
            $data = [
                'success'               => true,
                'title'                 => "Sucesso!",
